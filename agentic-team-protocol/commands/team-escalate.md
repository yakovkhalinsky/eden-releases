...
5. Write an `escalation_record`:
   ...
   "${EDEN_MEMORY_BIN}" remember \
     ...
     --metadata '{"kind":"escalation_record","stage":"escalation","goal_id":"'${GOAL_ID}'","owner_role":"dispatcher","mode":"'${MODE}'","promote_to_full":"'${PROMOTE_TO_FULL}'","claude_task_id":"'${CLAUDE_TASK_ID}'"}'
   ```
6. Route according to escalation levels and report the path to the user:
...
7. If `promote_to_full` is true, instruct the user to resume the goal with `/team-full` or `/team-continue ${GOAL_ID}` so the full lifecycle takes over.
8. Return the escalation record ID and the assigned routing level.
