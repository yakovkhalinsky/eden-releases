...
3. Write a `hand_off_record`:
   ...
   "${EDEN_MEMORY_BIN}" remember \
     ...
     --user-id "${USER_ID}" \
     --content "{\"kind\":\"hand_off_record\",\"goal_id\":\"${GOAL_ID}\",\"stage\":\"${CURRENT_STAGE}\",\"from_role\":\"${FROM_ROLE}\",\"to_role\":\"${TO_ROLE}\",\"reason\":\"${REASON}\",\"input_record_ids\":[\"${LATEST_RECORD_ID}\"],\"output_record_ids\":[\"${HAND_OFF_ID}\"],\"success_criteria\":\"${SUCCESS_CRITERIA}\",\"deadline\":\"${DEADLINE}\",\"escalation_trigger\":\"${ESCALATION_TRIGGER}\"}" \
     --metadata '{"kind":"hand_off_record","stage":"hand_off_or_closure","goal_id":"'"${GOAL_ID}"'","owner_role":"archivist"}'
   ```
4. Spawn the receiving role subagent with the hand-off payload and the full goal context.
