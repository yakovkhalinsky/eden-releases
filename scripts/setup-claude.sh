#!/usr/bin/env sh
# Set up the memory MCP server for Claude Code CLI.
# Usage: curl -fsSL https://0d3sa.com/memory/setup-claude.sh | sh
# Usage with explicit agent_id: curl -fsSL https://0d3sa.com/memory/setup-claude.sh | sh -s -- my-agent
#
...
CONFIG="{\"memory\":{\"command\":\"${COMMAND}\",\"args\":[\"--db\",\"${DB}\"],\"env\":{\"MEMORY_LOG_LEVEL\":\"INFO\",\"MEMORY_AGENT_ID\":\"${AGENT_ID}\"}}}"

claude config set mcpServers "${CONFIG}"

echo "Configured Claude Code MCP server 'memory' with command: ${COMMAND}"
echo "Database: ${DB}"
