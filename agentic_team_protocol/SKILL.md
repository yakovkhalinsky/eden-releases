...
1. Install `od3sa-memory` and make sure it is on your `PATH`:
   ```bash
   curl -fsSL https://0d3sa.com/memory/install.sh | sh
   ```
...
Run `od3sa-memory setup claude` in a project to generate or update the project `.env` with seed keys and per-role model comments.
...
- If Memory MCP tools are unavailable, use the `/memory-*` fallback slash commands or the `od3sa-memory` CLI with `--agent-id`, `--user-id`, `--org-id`, and `--workspace-id`.
...
2. The project `.env` file generated by `od3sa-memory setup claude`.
...
If the Memory MCP tools are unavailable, use the `/memory-*` fallback slash commands or invoke `od3sa-memory` directly from Bash. Restart Claude Code after `od3sa-memory setup claude` if commands are missing.

### Scope guard for direct CLI calls

Before any direct `od3sa-memory` call, verify that `org_id` and `workspace_id` are non-empty. Abort with an error instead of calling the CLI with empty scope values.
...
> The memory CLI rejects empty `--org-id`/`--workspace-id` values (merged to memory `master`). ATP never calls the CLI with empty scope; if you see unscoped records, check that `od3sa-memory` is up to date and that identity was resolved before the call.
