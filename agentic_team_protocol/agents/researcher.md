...
- Every `mcp__memory__memory_recall`, `memory_remember`, `memory_search`, `memory_search_semantic`, `memory_edit`, and `memory_forget` call must include explicit `org_id` and `workspace_id` from the project environment (`MEMORY_ORG_ID`, `MEMORY_WORKSPACE_ID`) or `agentic-team-config.yaml`.
...
3. Search Memory, read relevant files, and use WebSearch/WebFetch if needed.
4. Summarise findings, options, trade-offs, and confidence.
5. Record the IDs of any memories recalled via `memory_recall` or `memory_search` that shaped the summary in `recalled_memory_ids`.
6. If the chosen path is written into a plan file, record its absolute path in the context summary metadata (`plan_file_path`).
7. Store the context summary in Memory.
...
- Do not bury findings in conversation — always write them to Memory.
