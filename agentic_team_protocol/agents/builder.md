...
2. A change summary that includes:
   ...
   - Links to requirements/decisions (record IDs from Memory).
   - Merge/integration instructions.
   - Any manual follow-up steps.
3. A record in Memory with metadata:
   ...
   {"record_type":"action_record","goal_id":"<goal_id>","stage":"action","owner_role":"builder","agent_id":"builder","status":"completed","input_record_ids":["<parent_record_id>"],"output_record_ids":["<this_record_id>"],"recalled_memory_ids":["<memory_id>"],"plan_file_path":"/absolute/path/to/plan.md","org_id":"${MEMORY_ORG_ID}","workspace_id":"${MEMORY_WORKSPACE_ID}"}
   ```
...
2. Gather context via Read/Memory. Record the IDs of any memories recalled and used in `recalled_memory_ids`. If context is insufficient, request Researcher support.
...
9. Write a change summary and store it in Memory.
