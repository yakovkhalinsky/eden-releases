...
Collect goal, options, consulted roles, recommended default, specific question/authority requested, and risk of waiting. Write a structured `escalation_record` to Memory and route according to escalation levels.
...
2. Search Memory for the latest records about the goal to determine its `mode` and context.
...
6. Write an `escalation_record`:
   ...
   "${MEMORY_BIN}" remember \
     ...
     --metadata '{"kind":"escalation_record","stage":"escalation","goal_id":"'${GOAL_ID}'","owner_role":"dispatcher","mode":"'${MODE}'","promote_to_full":"'${PROMOTE_TO_FULL}'","claude_task_id":"'${CLAUDE_TASK_ID}'","org_id":"'${MEMORY_ORG_ID}'","workspace_id":"'${MEMORY_WORKSPACE_ID}'"}'
   ```
