...
2. When starting a new goal, pass the full user request to the Dispatcher. The Dispatcher records a `goal_record` (with `metadata.mode: lite`) and a `plan_record` in Memory, then routes directly to `builder` for everyday tasks.
...
- Multi-step or non-trivial goals must not be planned outside Memory; if plan mode is used, the resulting plan file path must be recorded in a `plan_record`, `context_summary`, or `action_record`.
