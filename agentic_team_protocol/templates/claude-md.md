...
2. Before any decision that touches user preferences, coding style, security, or tooling, call `mcp__memory__memory_recall` first.
3. After corrections, working solutions, or settled conventions, call `mcp__memory__memory_remember`.
...
6. If memory tools are unavailable, use the `/memory-*` fallback slash commands.

## Agentic Team Protocol rules

1. For every non-trivial task, start with the Dispatcher subagent, record a goal receipt in Memory, and obtain a dispatch instruction before any implementation work.
...
The parent assistant and every ATP role must keep the Claude Code task list aligned with the durable Memory trail:

- One task represents the whole goal. Its `claude_task_id` is stored in Memory record metadata.
...
Pause and surface the situation to the user only when the latest Memory record indicates one of:
