...
2. Before the action role starts, Builder or Runtime checks whether a worktree
   exists for the current `goal_id` in Memory (look for `worktree_path` and
...
Keep the Claude Code in-app task list in sync with the Memory trail. One
...
Every ATP Memory tool call (`memory_recall`, `memory_remember`, `memory_search`,
`memory_search_semantic`, `memory_edit`, `memory_forget`, and the `memory_health`/`memory_vacuum`
...
command must abort with an error rather than call memory with an empty scope.
The memory CLI currently accepts empty strings for `--org-id`/`--workspace-id`
and stores or fetches unscoped records; the long-term fix requires an memory
...
This prevents team-mode Memory calls from recalling memories that belong to
other workspaces.
