...
Primary: **a developer wiring `memory` into an MCP client** (Claude Code, Cursor, Hermes, or any stdio MCP client).
...
- Developers self-hosting sync, who need `relay` deployment and hardening guides (VPS, Tailscale, systemd).
...
`0d3sa.com` is the public documentation and release surface for `memory` and `relay` — a local-first memory layer for AI agents and its dedicated sync relay. It makes the binaries learnable, installable, and verifiable outside of the source repositories.
...
The site is the **primary, durable source of truth for memory and relay usage**, not a marketing wrapper. It competes with "read the repo README" by ordering the install/setup path first and treating `memory` and `relay` as two components of one coherent system.
...
- `memory` CI normally auto-cuts releases, updates the manifest, and regenerates `docs-site/src/data/downloads.json`.
- Skill pages are generated from `skills/*/SKILL.md` into `docs-site/src/content/docs/memory/skills/` via `scripts/generate-skills-site.py`.
- A typical user journey: curl-install `memory` → run `memory setup` in a project → restart Claude Code → remember and recall a fact. A secondary journey: install `relay` on a VPS → pair devices → run a sync loop.
...
- Install and setup instructions for `memory` and `relay`.
...
- Content is docs-first; the homepage and navigation must route developers to `memory` quickly.
...
- Name: `0d3sa.com` and `eden-releases`; product names `memory` and `relay`.
...
- `/home/yakov/git/eden-releases/docs-site/src/content/docs/` — docs structure for memory and relay.
...
1. **memory first.** The published site serves the developer who is installing and wiring memory into an MCP client; `relay` is reachable from that journey.
...
5. **One coherent system.** `memory` and `relay` are presented as two parts of one memory system, not unrelated products.
