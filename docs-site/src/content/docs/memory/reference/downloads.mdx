...
```bash
make release       # od3sa-memory for linux-amd64/arm64, darwin-amd64/arm64
make release-relay # od3sa-relay for linux-amd64/arm64, darwin-amd64/arm64
make release-all   # both of the above
```

This produces `dist/od3sa-memory-<platform>` and `dist/od3sa-relay-<platform>` for each supported platform.
