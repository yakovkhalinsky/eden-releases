---
title: Install Memory Claude skill
description: Use memory as a persistent memory layer inside Claude Code CLI.
content_type: reference
template: doc
skill_name: memory-claude
skill_version: 3.0.2
skill_tags: mcp, memory, claude-code, skill, prompt, subagent
...
# Install Memory Claude skill

Use memory as a persistent memory layer inside Claude Code CLI.

## Download this skill

The installable artifact is the raw `SKILL.md` file:

- [Download `memory-claude/SKILL.md`](/memory/skills/memory-claude/SKILL.md)
- Or fetch it from the terminal:

  ```bash
  curl -fsSL https://0d3sa.com/memory/skills/memory-claude/SKILL.md -o memory-claude/SKILL.md
  ```
...
For step-by-step client wiring, see [Connect Claude Code](/memory/tutorials/connect-claude-code/).
