...
Packets are normally rendered to stdout and forgotten. If you want one to outlive the session, build it with `--publish` (or the `memory_packet_publish` tool). A published packet is stored in the database as a durable record, so you can list and re-export it later without rebuilding:
...
A packet is always scoped to exactly one `org_id`/`workspace_id` pair. The server environment variables `MEMORY_ORG_ID` and `MEMORY_WORKSPACE_ID` are used by default; you can override them per call with `org_id` and `workspace_id` (CLI: `--org-id`/`--org`, `--workspace-id`/`--workspace`).
...
- [CLI reference](/memory/reference/cli/#packet)
