...
description: Remap org_id and workspace_id for a set of memory memories with memory_migrate.
content_type: how-to
---

# Migrate a workspace

When a project is renamed or moved to a new organization, you can update the `org_id` and `workspace_id` on every matching memory with `memory_migrate`. This guide shows how to preview and execute a scope remap.
...
`memory_migrate` defaults to dry-run. Call it first to see how many rows match without changing anything.
...
```json
{
  "path": "/home/yourname/backups/memory-pre-migrate.bin",
