...
description: Scoped soft-delete or hard-delete of memory memories with memory_prune.
content_type: how-to
---

# Prune old memories

Over time an memory database accumulates expired, duplicated, or out-of-scope memories. `memory_prune` lets you remove them in bulk, scoped to the identities and workspaces you choose. This guide shows soft deletion and optional permanent deletion.
...
`memory_prune` filters by any combination of:
...
Without `confirm: true`, `memory_prune` returns the matching count and sample IDs without deleting anything.
...
- `memory_health` reflects the reduced total count after hard deletion.
