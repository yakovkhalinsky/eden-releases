...
When a peer device replaces its identity sidecar, memory stages the new public key as a **pending key change**. You must inspect and approve it before the device can sync again. This guide shows how to list, approve, or reject pending changes.
