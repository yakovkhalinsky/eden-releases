...
description: Run a self-hosted relay on a public VPS with Let's Encrypt TLS, systemd, ufw, fail2ban, and certbot renewal hooks.
content_type: how-to
---

# Deploy on a public VPS

This guide walks through running a self-hosted relay on a public
...
server](/relay/how-to/run-relay-server/).
...
```bash
sudo useradd -r -s /usr/sbin/nologin -M relay
sudo mkdir -p /var/lib/relay /etc/relay /var/log/relay
sudo chown -R relay:relay /var/lib/relay /etc/relay /var/log/relay
sudo chmod 750 /var/lib/relay /etc/relay
...
| Path | Purpose |
|------|---------|
| `/var/lib/relay/relay.db` | SQLite database for the relay directory. |
| `/etc/relay/relay.env` | Environment variables loaded by systemd. |
| `/etc/relay/certbot-deploy-hook.sh` | certbot deploy hook that restarts the relay. |
| `/var/log/relay` | Service log directory (optional, when not using journald). |
...
```bash
sudo install -o relay -g relay -m 600 /dev/null /etc/relay/root-key-passphrase
openssl rand -base64 32 | sudo tee /etc/relay/root-key-passphrase > /dev/null
sudo chmod 600 /etc/relay/root-key-passphrase
sudo chown relay:relay /etc/relay/root-key-passphrase
```
...
Make sure the `relay` user can read them. The simplest approach is to add
`relay` to the `ssl-cert` group and ensure the group can read the
`/etc/letsencrypt/archive` and `/etc/letsencrypt/live` directories:

```bash
sudo usermod -a -G ssl-cert relay
...
```bash
sudo tee /etc/relay/relay.env <<'EOF'
# memory relay runtime configuration
MEMORY_LOG_LEVEL=INFO
MEMORY_LOG_FORMAT=json
EOF
sudo chown relay:relay /etc/relay/relay.env
...
```ini
[Unit]
Description=memory public relay
...
ReadOnlyPaths=/etc/relay /etc/letsencrypt
...
`--allow-remote-bind` (or `MEMORY_RELAY_ALLOW_REMOTE_BIND=1`) is set.
...
starting the loop by calling the `memory_relay_register` MCP tool from an agent
session.
...
```bash
sudo tee /etc/relay/certbot-deploy-hook.sh <<'EOF'
#!/bin/sh
set -e
systemctl restart relay
EOF
sudo chmod 755 /etc/relay/certbot-deploy-hook.sh
sudo chown root:root /etc/relay/certbot-deploy-hook.sh
```
...
```bash
sudo certbot renew --deploy-hook /etc/relay/certbot-deploy-hook.sh --dry-run
```
...
deploy_hook = /etc/relay/certbot-deploy-hook.sh
EOF
```
...
- **fail2ban**: Install and enable fail2ban to slow brute-force scans on SSH
  ...
  port = 443
  filter = relay
  logpath = /var/log/relay/access.log
  maxretry = 10
  bantime = 3600
  EOF
  ```

  Create `/etc/fail2ban/filter.d/relay.conf` with a basic filter:
  ...
  to monitor relay requests, route access logs to `/var/log/relay/access.log`
  or point fail2ban at the systemd journal for the `relay.service` unit.
...
- **Back up the relay database**: The relay database only stores device
  ...
  ```bash
  sudo -u relay sqlite3 /var/lib/relay/relay.db ".backup /var/lib/relay/relay.db.backup"
  ```
...
- The `relay` service is running under systemd as the `relay` user.
