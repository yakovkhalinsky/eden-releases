...
stream, analyze it, and store a durable `insights_report` Memory record
...
Memory through the current `MEMORY_GOAL_ID`.

## Components

| Component | Location |
|---|---|
| ATP supervisor | `/home/yakov/git/memory/eden-team` |
...
`/home/yakov/git/memory`. Build it with `make build-team` from the
`memory` repository; the binary is produced as `eden-team` in the repository
root. Update any local runbook paths and MCP configs from
`agentic_team_protocol/` to `cmd/eden-team/` inside `memory`.
...
3. Runs `eden-business-simulator insights <stream_id> --storage sqlite
   --output-dir <dir>`.
4. Exports `MEMORY_GOAL_ID` and `MEMORY_WORKSPACE_ID` into the subprocess
...
- `--goal-id` — defaults to `MEMORY_GOAL_ID` from the environment, falling back
...
| Variable | Purpose |
|---|---|
| `MEMORY_GOAL_ID` | ATP goal ID attached to the `insights_report` memory. |
| `MEMORY_BUSINESS_SIMULATOR_DIR` | Override the simulator checkout path. |
| `MEMORY_DB` | Override the Memory SQLite path. |
| `MEMORY_WORKSPACE_ID` | The script overrides this with the simulator directory so memories are stored in the simulator workspace. |

## Manual / standalone run

```bash
export MEMORY_GOAL_ID=76fbaeac-698b-444f-82f9-4f77aa431e54
...
execute the script. `eden-team` forwards the generated `MEMORY_GOAL_ID` to the role
process, and the script propagates it to the simulator CLI:

```bash
/home/yakov/git/memory/eden-team start \
  ...
  --mcp-config /home/yakov/git/memory/cmd/eden-team/mcp.json \
  --dangerously-skip-permissions \
  --verbose
```

Because the script reads `MEMORY_GOAL_ID` from the environment, the resulting
...
memory --db ~/.memory/default.db recall \
...
4. An `insights_report` Memory record exists and references the ATP goal
