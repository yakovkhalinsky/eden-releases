...
Position the Agentic Team Protocol (ATP) as the installable product in the 0d3sa docs site, and reframe memory, relay, and eden-team as the components that make it work.

## Current problems

1. `src/pages/index.astro` leads with "Local-first memory for AI agents" and sends the primary CTA to `/memory/getting-started/`.
2. `src/pages/docs.astro` presents memory and agentic-team-protocol as two equal products.
3. `src/content/docs/agentic-team-protocol/index.mdx` is a plain text page without a splash hero.
4. `astro.config.mjs` lists memory first with a full Diátaxis tree; agentic-team-protocol is second and smaller.
5. `src/content/docs/memory/index.mdx` frames memory as the "flagship binary of the memory monorepo".
6. Detailed relay and headless-supervisor content is mixed into memory and ATP sections.
...
- Install card shows both ATP and memory install commands, framed as "Step 1 / Step 2".
- Product cards: ATP first/primary, memory as "memory & sync component", relay/eden-team grouped as infrastructure components.
- Three-binary section retitled "What ships in the release".
- Decision card short answer: "Install ATP. It brings memory with it."

### B. Docs landing page (`src/pages/docs.astro`)

- ATP as hero/default path with full Diátaxis entry points.
- "Components" section below with cards for memory, relay, eden-team linking to their deep docs.
- Remove the two-equal-products framing.

### C. ATP overview (`src/content/docs/agentic-team-protocol/index.mdx`)

- Convert to Starlight `template: splash` with `hero` block matching memory visual weight.
...
2. **Components** — new group containing:
   - **memory** (Overview, Quick start, Tutorials, How-to, Concepts, Reference)
   - **relay** (Overview, How-to guides)
   - **eden-team** (Overview, Tutorials)

Moves and redirects:
- `/memory/how-to/run-relay-server/` → `/relay/how-to/run-relay-server/` (redirect added)
- `/memory/how-to/deploy-public-vps/` → `/relay/how-to/deploy-public-vps/` (redirect added)
- `/agentic-team-protocol/tutorials/headless-supervisor/` → `/eden-team/tutorials/headless-supervisor/` (redirect added)

### E. memory overview (`src/content/docs/memory/index.mdx`)

- Frame memory as the durable memory/sync component of ATP.
- Remove "flagship binary of the memory monorepo" framing.
...
- `src/content/docs/relay/index.mdx` — overview splash or concept page.
- `src/content/docs/relay/how-to/run-relay-server.md` — moved from memory.
- `src/content/docs/relay/how-to/deploy-public-vps.md` — moved from memory.
...
- Update `multi-device-sync.md` to point to new relay how-to paths.
...
- [x] Rewrote `src/content/docs/memory/index.mdx` as ATP component.
- [x] Created `relay/` and `eden-team/` sections; moved run-relay-server, deploy-public-vps, and headless-supervisor pages.
